// Pending Work History Page - Client Component
'use client'
...
import { FileClock, ArrowLeft, Search, ChevronLeft, ChevronRight, RefreshCw, Download } from 'lucide-react'
...
interface PendingHistoryRecord {
    id: string; created_at: string; start_date: string; end_date: string; time: string | null
    vendor_name: string; work_content: string; unit: string; engineering_contact: string; note: string | null
}

export default function PendingHistoryClient() {
    const router = useRouter()
    const supabase = createClient()
    const { toast } = useToast()
    const [data, setData] = useState<PendingHistoryRecord[]>([])
    ...
    const fetchData = async () => {
        ...
        if (keyword.trim()) q = q.or(`vendor_name.ilike.%${keyword}%,work_content.ilike.%${keyword}%,unit.ilike.%${keyword}%`)
        ...
        <div className="min-h-screen bg-slate-100">
            <header className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center shadow-sm sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-lg font-black text-slate-800 flex items-center gap-2"><FileClock className="w-5 h-5 text-purple-500" />待處理工作歷史記錄</h1>
                </div>
            </header>
            <main className="p-6">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl shadow-sm border border-slate-200">
                    <div className="p-4 border-b border-slate-100">
                        <div className="flex flex-wrap items-end gap-4">
                            ...
                            <div className="space-y-1"><Label className="text-xs text-slate-500">關鍵字搜尋</Label><Input type="text" placeholder="廠商、單位、施工內容..." value={keyword} onChange={(e) => setKeyword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch()} className="w-60" /></div>
                            ...
                            <div className="flex items-center gap-2"><Label className="text-xs text-slate-500">每頁顯示</Label><Select value={String(pageSize)} onValueChange={(v) => { setPageSize(Number(v)); setPage(1) }}><SelectTrigger className="w-20"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="10">10</SelectItem><SelectItem value="50">50</SelectItem><SelectItem value="100">100</SelectItem></SelectContent></Select></div>
                    ...
                    <div className="overflow-x-auto">
                        <Table>
                            <TableHeader><TableRow>
                                ...
                                <TableHead className="w-12">#</TableHead><TableHead className="text-xs">ID</TableHead><TableHead>建立時間</TableHead><TableHead>開始日期</TableHead><TableHead>結束日期</TableHead><TableHead>時間</TableHead><TableHead>廠商</TableHead><TableHead>單位</TableHead><TableHead>負責人</TableHead><TableHead>施工內容</TableHead><TableHead>備註</TableHead>
                            ...
                            <TableBody>
                                {loading ? <TableRow><TableCell colSpan={12} className="text-center py-8"><RefreshCw className="w-5 h-5 animate-spin mx-auto text-slate-400" /></TableCell></TableRow>
                                    : data.length === 0 ? <TableRow><TableCell colSpan={12} className="text-center py-8 text-slate-400">查無資料</TableCell></TableRow>
                                        : data.map((row, index) => (
                                            <TableRow key={row.id} className={`hover:bg-purple-50/50 ${selected.has(row.id) ? 'bg-purple-100' : ''}`}>
                                                ...
                                                <TableCell className="font-mono">{row.start_date}</TableCell><TableCell className="font-mono">{row.end_date}</TableCell><TableCell className="font-mono">{row.time?.slice(0, 5) || '-'}</TableCell>
                                                <TableCell className="font-bold">{row.vendor_name}</TableCell><TableCell><Badge variant="outline">{row.unit}</Badge></TableCell><TableCell>{row.engineering_contact}</TableCell>
