...
export default function VendorWorkNewPage() {
    ...
    const { register, handleSubmit, watch, trigger, setValue, getValues, formState: { errors, isSubmitting } } = useForm<VendorWorkFormValues>({
    ...
    // 借物相關 state（到院）
    const [borrowAction, setBorrowAction] = useState<'borrow' | 'none'>('none')
    const [borrowedItems, setBorrowedItems] = useState<string[]>([])
    const [borrowedOtherText, setBorrowedOtherText] = useState('')
    const [lenderName, setLenderName] = useState('')

    // 歸還相關 state（離院）
    const [returnAction, setReturnAction] = useState<'return' | 'none'>('none')
    const [returnedItems, setReturnedItems] = useState<string[]>([])
    const [returnedOtherText, setReturnedOtherText] = useState('')
    const [receiverName, setReceiverName] = useState('')
    const [refArrivalId, setRefArrivalId] = useState<string | null>(null)
    ...
    // 離院時，根據廠商名稱自動查詢今日到院的借物紀錄
    const fetchArrivalBorrow = useCallback(async (name: string) => {
        if (!name || entryStatus !== 'departure') return
        setIsFetchingArrival(true)
        const today = new Date().toLocaleDateString('sv-SE', { timeZone: 'Asia/Taipei' })
        const { data } = await supabase
            .from('vendor_today_work')
            .select('id, borrow_action, borrowed_items, lender_name')
            .eq('vendor_name', name)
            .eq('work_date', today)
            .eq('entry_status', 'arrival')
            .eq('borrow_action', 'borrow')
            .order('created_at', { ascending: false })
            .limit(1)
            .single()
        setIsFetchingArrival(false)
        if (data) {
            setRefArrivalId(data.id)
            setReturnAction('return')
            const items = (data.borrowed_items as any)?.items || []
            setReturnedItems(items)
            setReturnedOtherText((data.borrowed_items as any)?.other_text || '')
            setOriginalBorrowedItems(items)
            setOriginalBorrowedOtherText((data.borrowed_items as any)?.other_text || '')
            setLenderName(data.lender_name || '')
            toast({ title: '已帶入借物紀錄', description: `偵測到該廠商今日借用了 ${items.join('、')}，請確認歸還狀況。` })
        } else {
            setRefArrivalId(null)
            setReturnAction('none')
            setReturnedItems([])
            setOriginalBorrowedItems(null)
            setOriginalBorrowedOtherText('')
        }
    }, [entryStatus, supabase, toast])
        ...
        try {
            ...
            if (pendingData.entry_status === 'arrival') {
                ...
                if (borrowAction === 'borrow') {
                    payload.borrowed_items = { items: borrowedItems, other_text: borrowedOtherText }
                ...
                if (payload.borrow_action === 'return' || payload.borrow_action === 'partial_return') {
                    payload.returned_items = { items: returnedItems, other_text: returnedOtherText }
                    payload.receiver_name = receiverName
                    payload.ref_arrival_id = refArrivalId
                            ...
                            <CardContent className="space-y-4">
                                <FormField label="廠商名稱" required error={errors.vendor_name?.message} touched={touchedFields.vendor_name}>
                                    <Input
                                        ...
                                        placeholder="請輸入廠商名稱"
                                        onBlur={() => {
                                            handleFieldBlur('vendor_name')
                                            // 離院時：失焦後自動查詢今日借物紀錄
                                            if (entryStatus === 'departure') fetchArrivalBorrow(vendorName || '')
                                        }}
                                    />
                                </FormField>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <Card className="border-green-200 bg-green-50/50 dark:border-green-900/50 dark:bg-green-950/20">
                                    <CardHeader>
                                        <CardTitle className="text-base flex items-center gap-2 text-green-700 dark:text-green-400">
                                            <Package className="w-4 h-4" />
                                            歸還資訊
                                            {isFetchingArrival && <span className="text-xs font-normal text-muted-foreground ml-2">查詢借物紀錄中...</span>}
                                        </CardTitle>
                                    </CardHeader>
                                    <CardContent className="space-y-4">
                                        {/* 歸還狀態選擇 */}
